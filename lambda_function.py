...
dynamodb_table = dynamodb.Table('Input_Order_Confirmation_Request')

status_check_path = '/status'
employee_path = '/Current_SKU'
employees_path = '/Current_SKUs'
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if http_method == 'GET' and path == status_check_path:
            ...
            employee_id = event['queryStringParameters']['Product_id']
            ...
            response = modify_employee(body['Product_id'], body['updateKey'], body['updateValue'])
        elif http_method == 'DELETE' and path == employee_path:
            body = json.loads(event['body'])
            response = delete_employee(body['Product_id'])
        else:
...
def get_employee(employee_id):
    try:
        response = dynamodb_table.get_item(Key={'Product_id': employee_id})
